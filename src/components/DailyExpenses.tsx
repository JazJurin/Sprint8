...
export default function DailyExpenses() {
  ...
  const dailyExpenses = useSelector(
    (state: RootState) => state.DailyExpenses.DailyExpenses
  );
  const yesterdayExpenses = useSelector(
    (state: RootState) => state.DailyExpenses.YesterdayExpenses
  );
    ...
    const fetchExpensesData = async () => {
      try {
        ...
        if (responseData && responseData.week) {
          ...
          console.error(
            "Invalid expenses data format or no data for today:",
            responseData
          );
        }
        ...
        const yesterdayIndex =
          (dayIndex - 1 + daysOfWeek.length) % daysOfWeek.length;
        ...
        if (yesterdayName in responseData.week) {
          ...
          console.error(
            "Error in yesterday's expenses or no data for yesterday:",
            responseData
          );
        }
      } catch (error) {
  ...
  const variationPercentage =
    ((dailyExpenses - yesterdayExpenses) / (yesterdayExpenses || 1)) * 100;
